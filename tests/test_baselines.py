...
def test_add_weather_baseline_features():
    ...
    assert out["forecast_event_indicator"][0] == 1
    assert out["microprice_minus_mid"][0] == 1.0


def test_add_weather_baseline_features_less_than_contract():
    df = pl.DataFrame(
        {
            "forecast_temperature": [78.0],
            "threshold_value": [79.0],
            "threshold_upper_value": [None],
            "comparison_operator": ["<"],
            "contract_type": ["threshold"],
        }
    ...
    assert out["forecast_above_threshold"][0] == 0
    assert out["forecast_event_indicator"][0] == 1
